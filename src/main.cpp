    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          int len = ptsx.size();
          Eigen::MatrixXd waypoints = Eigen::MatrixXd(2, len);
          for (int i = 0; i < len; i++) 
          {
            double dx = ptsx[i] - px;
            double dy = ptsy[i] - py;
          ...
          //Polynomial fits
          Eigen::VectorXd ptsx_e = waypoints.row(0);
          Eigen::VectorXd ptsy_e = waypoints.row(1);
          auto coeffs = polyfit(ptsx_e, ptsy_e, 3);
          //Calculate current error
          double cte = polyeval(coeffs, 0) - 0; //px =0 py =0;
          double epsi = 0 - atan(coeffs[1]);     //psi = 0
          //Get State vector
